...
class VerReseniasPlatilloActivity : AppCompatActivity() {
    var idPlatillo=""
    var tbProductos: TableLayout?=null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_ver_resenias_platillo)
        idPlatillo=intent.getStringExtra("idPlatillo").toString()
        tbProductos=findViewById(R.id.tbProductos)
        llenarTabla()
    }

    fun llenarTabla(){
        val ID_PLATILLO=idPlatillo
        tbProductos?.removeAllViews()
        var queue= Volley.newRequestQueue(this)
        var url="https://promofood.000webhostapp.com/PromoFoodMYSQL/mostrarReseniasPlatillo.php?idPlatillo=$ID_PLATILLO"
